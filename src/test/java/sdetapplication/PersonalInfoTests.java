...
public class PersonalInfoTests {
	...
	public void submitFullApplication() {
		...
		//certifications(certifications, "Java OCA");
	...
	public void certifications(List<String> certifications, String cert) {
		for(int i=0; i<certifications.size();i++) {
			if(certifications.get(i).equals("AWS")) {
				driver.findElement(By.xpath("//label[@class='checkChoice']")).click();
				
			}
			else if(certifications.get(i).equals("Java OCA")) {
				driver.findElement(By.xpath("//input[@id='Checkbox_1']")).click();
		}
			else if (certifications.get(i).equals("Scrum Master")) {
				driver.findElement(By.xpath("//span[@class='multiAttType']")).click();
		}
